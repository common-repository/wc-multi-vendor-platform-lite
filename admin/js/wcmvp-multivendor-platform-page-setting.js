...
(function( $ ) {
    'use strict';
    var wcmvp_url, wcmvp_split_url, wcmvp_page_setting_dashboard, wcmvp_page_my_orders, wcmvp_page_terms_conditions
    ,wcmvp_page_store_listing,wcmvp_index_elem;
    if(window.location.href != ""){
    wcmvp_url = window.location.href;
    wcmvp_split_url = wcmvp_url.split("#");
    wcmvp_index_elem = [0,1,2,3,5,6,7];
    }
        $(document).ready(function( $ ){
            if(wcmvp_split_url[1] == 'page-setting'){
                ...
                $(document).find('#wcmvp-page-setting').css('display','block');
                ...
                $(document).find('#wcmvp-setting-page-setting').addClass('submenu-tab-active');
                ...
                $(document).find('.wcmvp_list_float').eq(4).css({'background':'white','border':'5px solid white','box-shadow':'0px 3px 3px -2px rgba(0, 0, 0, 0.2),0px 3px 4px 0px rgba(0, 0, 0, 0.14),0px 1px 8px 0px rgba(0,0,0,.12)' ,'border-bottom-style':'solid'});

            }
			$('#wcmvp-setting-page-setting').on('click',function(){
                $(document).find('#wpbody-content').show();
                $(document).find('#wcmvp-admin-all-products').removeClass('nav-tab-active');
                $(document).find('.wcmvp-setting-section').children().hide();
				$(document).find('#wcmvp-page-setting').css('display','block');
            ...
            $('#wcmvp-page-setting-submit').on('click',function(){
                $('#wcmvp-loader-image').css('display','block');
                wcmvp_page_setting_dashboard = $('#wcmvp_page_setting_dashboard').children('option:selected').val();
                wcmvp_page_my_orders = $('#wcmvp_page_my_orders').children('option:selected').val();
                wcmvp_page_terms_conditions = $('#wcmvp_page_terms_conditions').children('option:selected').val();
                wcmvp_page_store_listing = $('#wcmvp_page_store_listing').children('option:selected').val();

                var wcmvp_page_setting_data_array = {}
               
                $(document).find('.wcmvp_page_setting_page_data').each(function(wcmvp_serial, wcmvp_type){
 
                    if($(this).is("input"))
                    {
                        if($(this).attr('type') == 'text' )
                        {
                            wcmvp_page_setting_data_array[$(this).attr('id')] = $(this).val();
                        }
                        ...
                        else if($(this).attr('type') == 'checkbox' )
                        {
                            if($(this).is(':checked'))
                            {
                                wcmvp_page_setting_data_array[$(this).attr('id')] = 1;
                            }
                            else
                            {
                                wcmvp_page_setting_data_array[$(this).attr('id')] = 0;
                            }
                        }
                    }
                    else if($(this).is("select"))
                    {
                        wcmvp_page_setting_data_array[$(this).attr('id')] = $(this).children('option:selected').val();
                    }
                })
 
                var wcmvp_page_setting_data = {
                    'action' : 'wcmvp_page_setting', 
                    'wcmvp_page_setting[wcmvp_page_setting_dashboard]' : wcmvp_page_setting_dashboard,
                    'wcmvp_page_setting[wcmvp_page_my_orders]' : wcmvp_page_my_orders,
                    'wcmvp_page_setting[wcmvp_page_store_listing]' : wcmvp_page_store_listing,
                    'wcmvp_page_setting[wcmvp_page_terms_conditions]' : wcmvp_page_terms_conditions,
                    'wcmvp_page_setting' : wcmvp_page_setting_data_array,
                    'wcmvp_page_setting_nonce_verify' : wcmvp_page_setting_object.wcmvp_page_setting_nonce,

                }

                jQuery.post( wcmvp_page_setting_object.wcmvp_ajax_url, wcmvp_page_setting_data, function(response){
