...
(function( $ ) {
    ...
    wcmvp_index_elem=[0,2,3,4,5,6,7]
    }
        $(document).ready(function( $ ){

            if(wcmvp_split_url[1] == 'selling-option'){
                ...
                $(document).find('#wcmvp-setting-selling').addClass('submenu-tab-active');
                ...
                $(document).find('.wcmvp_list_float').eq(1).css({'background':'white','border':'5px solid white','box-shadow':'0px 3px 3px -2px rgba(0, 0, 0, 0.2),0px 3px 4px 0px rgba(0, 0, 0, 0.14),0px 1px 8px 0px rgba(0,0,0,.12)' ,'border-bottom-style':'solid'});

                
            }
            
			$('#wcmvp-setting-selling').on('click',function(){
                $(document).find('#wpbody-content').show();
                $(document).find('#wcmvp-admin-all-products').removeClass('nav-tab-active');
                $(document).find('.wcmvp-setting-section').children().hide();
				$(document).find('#wcmvp-selling-options').css('display','block');
            ...
            $('#wcmvp-selling-page-submit').on('click',function(e){
                e.preventDefault();

                $('#wcmvp-loader-image').css('display','block');

               var wcmvp_selling_option_data_array = {}
               
               $(document).find('.wcmvp_selling_option_page_data').each(function(wcmvp_serial, wcmvp_type){

                if($(this).is("input"))
                {
                    if($(this).attr('type') == 'text' )
                    {
                        wcmvp_selling_option_data_array[$(this).attr('id')] = $(this).val();
                    }
                    else if($(this).attr('type') == 'checkbox' )
                    {
                        if($(this).is(':checked'))
                        {
                            wcmvp_selling_option_data_array[$(this).attr('id')] = 1;
                        }
                        else
                        {
                            wcmvp_selling_option_data_array[$(this).attr('id')] = 0;
                        }
                    }
                }
                else if($(this).is("select"))
                {
                    wcmvp_selling_option_data_array[$(this).attr('id')] = $(this).children('option:selected').val();
                }
               })
	 
               var wcmvp_selling_option_data = {
                   'action' : 'wcmvp_selling_options_page',
                   'wcmvp_selling_options' : wcmvp_selling_option_data_array,
                   'wcmvp_sellings_option_nonce_verify' : wcmvp_sellings_options_object.wcmvp_sellings_option_nonce
               }
            
               jQuery.post( wcmvp_sellings_options_object.wcmvp_ajax_url, wcmvp_selling_option_data,
                function( response ){
